...
import { Cpu, Layers, SlidersHorizontal, MemoryStick, ListFilter, Server, Timer, Gauge, Network, Workflow, Calculator } from 'lucide-react';
...
function MapView({onOpen,completed,current,reduced}){
  ...
  return <main className="map-page"><div className="map-intro"><div><p className="eyebrow">{event.name} <span>/</span> {event.subtitle}</p><h1>Inferencia de LLMs<br/><em>en local.</em></h1><div className="map-intro-actions"><button className="hero-start" onClick={()=>onOpen('01')}><span className="hero-start-long">Empezar el taller</span><span className="hero-start-short">Empezar</span><Icon name="arrow" size={20}/></button></div></div><div className="map-intro-side"><strong>Eduardo Zapatero<br/>Álvaro García Pizarro <small>@lvrpiz</small></strong></div></div>
    <div className={'map-area '+(mobileVisual?'show-mobile-visual':'')}><div className="map-viewport" ref={viewport} tabIndex={0} role="group" aria-label="Mapa del taller. Flechas para elegir, Intro para abrir, más y menos para zoom, cero para ajustar." onKeyDown={onMapKey} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={onPointerUp} onPointerCancel={onPointerUp}><div className="map-world" style={{width:WORLD.width,height:WORLD.height,transform:`translate(${view.x}px, ${view.y}px) scale(${view.scale})`,transition:reduced||interacted.current?'none':'transform 270ms cubic-bezier(.2,.8,.2,1)'}}><div className="stage-band fundamentals"><span>01 / FUNDAMENTOS</span></div><div className="stage-band serving"><span>02 / SERVICIO</span></div><div className="stage-band production"><span>03 / PRODUCCIÓN</span></div><svg className="map-links" width="1600" height="920" viewBox="0 0 1600 920" aria-hidden="true"><path d="M175 238 C280 170 355 170 460 194 S630 250 735 244 S905 150 1010 195 S1220 230 1315 252 C1460 350 1415 425 1290 492 C1120 555 985 500 805 492 S490 475 323 492 C145 550 130 685 260 738 C420 805 580 720 775 734 S1110 705 1290 738"/></svg>{sections.map((section,index)=>{const[x,y]=positions[index],SectionIcon=sectionIcons[index];return <button key={section.id} type="button" className={'map-node '+(current===section.id?'current ':'')+(selected===index?'selected ':'')+(completed.includes(section.id)?'done':'')} style={{left:x,top:y}} onClick={()=>openNode(index)} aria-label={`Sección ${Number(section.id)}: ${section.title}${completed.includes(section.id)?', visitada':''}`}><span className="node-number">{section.id}</span><span className="node-label">{section.short}</span><span className="node-status"><SectionIcon size={22} strokeWidth={1.5} aria-hidden="true"/></span></button>;})}</div></div><div className="map-tools"><button onClick={()=>zoomCenter(1.25)} aria-label="Acercar mapa" title="Acercar"><Icon name="plus"/></button><button onClick={()=>zoomCenter(.8)} aria-label="Alejar mapa" title="Alejar"><Icon name="minus"/></button><button onClick={fit} aria-label="Ajustar mapa a la pantalla" title="Ajustar"><Icon name="fit"/></button></div><div className="map-hint">Selecciona una parada · Arrastra para mover el mapa</div><div className="mobile-map-toggle"><button onClick={()=>setMobileVisual(v=>!v)}>{mobileVisual?'Ver lista':'Ver mapa visual'}</button></div><div className="mobile-map-list"><SectionList onOpen={onOpen} completed={completed}/></div></div>
...
function SectionList({onOpen,completed}){return <ol className="section-list">{stages.map(stage=><li key={stage.id}><h3>{stage.name}</h3><ol>{sections.filter(s=>s.stage===stage.id).map(s=><li key={s.id}><button onClick={()=>onOpen(s.id)}><span>{s.id}</span>{s.title}{completed.includes(s.id)&&<Icon name="check" size={16}/>}</button></li>)}</ol></li>)}</ol>;}
...
export default function App(){
  ...
  useEffect(()=>{const onKey=e=>{if(e.key==='Escape'&&presentation){showMap();return;}if(e.target instanceof HTMLElement&&e.target.closest('input, textarea, select, summary, [contenteditable="true"], .workshop-support button'))return;if(presentation&&sectionId){if(e.key==='ArrowRight'||e.key==='PageDown'){e.preventDefault();move(1);}if(e.key==='ArrowLeft'||e.key==='PageUp'){e.preventDefault();move(-1);}}};window.addEventListener('keydown',onKey);return()=>window.removeEventListener('keydown',onKey);});
  const section=sectionId?sectionById[sectionId]:null;
  return <div className={'app '+(presentation?'presentation':'')}><header className="site-header"><button className="brand" onClick={showMap} aria-label="Ir al mapa del taller"><img className="brand-mark" src="/gdg-icon.svg" alt=""/><span className="brand-desktop">VallaTech Summit <b>/</b> Inferencia local</span><span className="brand-mobile">Inferencia local</span></button><div className="header-actions">{presentation&&<button className="explanation-trigger" onClick={()=>setPresentation(false)} aria-label="Ver explicación de esta sección">Explicación</button>}{section&&<button className="header-map" onClick={showMap}><Icon name="map" size={18}/> <span>Mapa</span></button>}<button className="presentation-trigger" aria-label={presentation?'Salir de presentación y volver al mapa':'Entrar en modo presentación'} onClick={()=>{if(presentation){showMap();return;}if(!sectionId)openSection('01');setPresentation(true);}}><Icon name="screen" size={18}/><span>{presentation?'Salir al mapa':'Presentar'}</span></button></div></header><AnimatePresence mode="wait" initial={false}><motion.div className="workspace" key={section?.id||'mapa'} initial={{opacity:0,y:reduced?0:9}} animate={{opacity:1,y:0}} exit={{opacity:0,y:reduced?0:-6}} transition={{duration:reduced?0:.2,ease:[.22,1,.36,1]}}>{section?<SectionView section={section} presentation={presentation} onNext={()=>move(1)} onMap={showMap}/>:<MapView onOpen={openSection} completed={completed} current={completed.at(-1)} reduced={reduced}/>}</motion.div></AnimatePresence>{section?<nav className="presentation-nav" aria-label="Navegación de secciones"><button onClick={()=>move(-1)} disabled={section.id==='01'}><Icon name="back" size={18}/><span>Anterior</span></button><Progress current={section.id} completed={completed}/><button onClick={()=>move(1)} disabled={section.id==='11'}><span>Siguiente</span><Icon name="arrow" size={18}/></button></nav>:<div className="overview-footer"><a className="event-link" href={event.url} target="_blank" rel="noreferrer" aria-label={`${event.date} · ${event.place}`}><span className="event-full">{event.date} · {event.place}</span><span className="event-mobile">13 nov 2026 · Escuela de Ingeniería Informática<br/>Universidad de Valladolid</span><span aria-hidden="true"> ↗</span></a><Progress current={null} completed={completed}/></div>}</div>;
}
